...
//gotta do -> strip off leading 0 in secret key before doing substring for first 16 chars-DONE.......... AND pad with leading 0 while getting CRC if length<8
...
int handleErrors(void)
{
 ...
 char *obfuscate(char *indata){
   ...
   unsigned char *base64decode = NULL; 
	...
	if (strlen(crc1_hex) < 8 && strlen(crc1_hex) > 5){
		char crc1_hex_temp[8];
		if (strlen(crc1_hex) == 7){
			strcpy(crc1_hex_temp, "0");
		}
		else if (strlen(crc1_hex) == 6){
			strcpy(crc1_hex_temp, "00");
		}
		strcat(crc1_hex_temp, crc1_hex);
		sprintf(crc1_hex, "%s", crc1_hex_temp);
	}
	...
	if (strlen(crc2_hex) < 8 && strlen(crc2_hex) > 5){
		char crc2_hex_temp[8];
		if (strlen(crc2_hex) == 7){
			strcpy(crc2_hex_temp, "0");
		}
		else if (strlen(crc2_hex) == 6){
			strcpy(crc2_hex_temp, "00");
		}
		strcat(crc2_hex_temp, crc2_hex);
		sprintf(crc2_hex, "%s", crc2_hex_temp);
	}
	...
	//writing data to text files, to run testcases with the java deobfuscation code, to see everything everything is working properly
	
	// creating file pointer to work with files
    FILE *fptr;

    fptr = fopen("/home/deebthik/Desktop/obfuscation_test/obfuscated.txt", "w");
